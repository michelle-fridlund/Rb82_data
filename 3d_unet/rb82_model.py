...
class NetworkModel(object):
    ...
    def model_train(self,args,model_outname,x,y,z,d,data_path,epoch,batch_size, \
        ...
        self.generator_t = self.generator_train(args)
        self.generator_v = self.generator_validate(args)
        # Generators
        self.data_train_gen = tf.data.Dataset.from_generator(self.generator_t, \
                                                        output_types=(tf.float32,tf.float32), \
                                                        output_shapes=(tf.TensorShape((128, 128, 16, 2)),tf.TensorShape((128, 128, 16, 1))))
        self.data_valid_gen = tf.data.Dataset.from_generator(self.generator_v, \
                                                        output_types=(tf.float32,tf.float32), \
                                                        output_shapes=(tf.TensorShape((128, 128, 16, 2)),tf.TensorShape((128, 128, 16, 1))))
        ...
        self.model.fit_generator(generator = self.data_train_gen,
                  steps_per_epoch = self.n_batches*(self.image_size-self.patch_size), #patches per patient
    ...
    def train(self, args):
        ...
        self.model_outname = self.model_name+"_e"+str(self.epoch)+"_bz"+str(self.batch_size)+"_lr"+str(self.lr)

        if os.path.exists(self.model_outname+".h5"):
            print("Model %s exists" % self.model_outname)
      
        #Initialise training
        self.model_train(args,self.model_outname,128,128,16,2,self.data_path,self.epoch,self.batch_size,self.lr)

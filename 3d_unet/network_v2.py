...
def construct_3D_unet_architecture(X):
    ...
    # ENCODING
    block1, dblock1 = conv_block(X, 64, .1)  # 32 kernels  # 32x32x16
    block2, dblock2 = conv_block(dblock1, 128, .1)  # 64x64x8
    block3, dblock3 = conv_block(dblock2, 256, .2)  # 128x128x4
    block4, dblock4 = conv_block(dblock3, 512, .2)

    block5, _ = conv_block(dblock4, 1024, .3, downsample=False)

    # DECODING
    block6 = convt_block(block5, block4, 512)
    block7, _ = conv_block(block6, 512, .3, downsample=False)

    block8 = convt_block(block7, block3, 256)
    block9, _ = conv_block(block8, 256, .2, downsample=False)

    block10 = convt_block(block9, block2, 128)
    block11, _ = conv_block(block10, 128, .2, downsample=False)

    block12 = convt_block(block11, block1, 64)
    block13, _ = conv_block(block12, 64, .1, downsample=False)
